...
_SERVER_INSTRUCTIONS = (
    ...
    "command/read/search/change, locate Harness tools if deferred/omitted; tool discovery is the "
    "only allowed pre-status action. Use Russian in Task title/summary/next_step and Knowledge "
    ...
    "or the path is known. Explicit identifiers/quoted literals or Knowledge/Task retrieval: use "
    "project_search before broad native work. Natural-language code/doc discovery may use native "
    "search directly; lexical hits do not block broad fallback. Exact path may skip search, not "
    "Task. Complete untruncated exact_coverage replaces native search for that needle. A code/doc "
