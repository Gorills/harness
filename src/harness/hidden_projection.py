...
Do not `git add -f` ignored files. Do not edit `.gitignore` or tracked instruction files to hide Harness artifacts. Do not delete or edit this Harness-owned Hidden rule.
...
def apply_hidden_projection(
    ...
    *,
    ...
    surfaces = hidden_instruction_surfaces(profiles)
    if not surfaces:
    ...
    return HiddenProjectionResult(
        ...
        projected_paths=tuple(path.as_posix() for path in sorted(desired, key=str)),
...
def remove_hidden_projection(
    ...
    *,
    ...
    surfaces = (
        hidden_instruction_surfaces(profiles)
        if profiles
...
def inspect_hidden_workspace(
    ...
    *,
    ...
    orphans: list[str] = []
    if expect_hidden:
        for surface in hidden_instruction_surfaces(required_profiles):
...
def _surface_for_profile(profile: str) -> HiddenInstructionSurface:
    if profile == CURSOR_PROFILE:
        return HiddenInstructionSurface(
            ...
            rule_bytes=("---\nalwaysApply: true\n---\n\n" + _HIDDEN_RULE_BODY).encode("utf-8"),
        )
    if profile == CLAUDE_CODE_PROFILE:
        return HiddenInstructionSurface(
            ...
            rule_bytes=_HIDDEN_RULE_BODY.encode("utf-8"),
        )
