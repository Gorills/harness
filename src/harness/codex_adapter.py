...
_OWNER_VERSION = 1
_AGENTS_FILE = "AGENTS.override.md"
_AGENTS_OWNER_MARKER = ".harness-agents-owner.json"
...
_AGENTS_EXCLUDE_BEGIN = "# BEGIN HARNESS CODEX AGENTS"
_AGENTS_EXCLUDE_END = "# END HARNESS CODEX AGENTS"
_AGENTS_EXCLUDE_BODY = ("/AGENTS.override.md", f"/.codex/{_AGENTS_OWNER_MARKER}")
_AGENTS_GENERATED_HEADER = (
    "<!-- Generated by Harness; ownership: .codex/.harness-agents-owner.json. -->\n"
)
_ROOT_AGENTS_READ_INSTRUCTION = (
...
class CodexProjectRegistrationDiagnostic:
    ...
    root_agents_state: HostRegistrationState | None = None
...
class CodexAdapter:
    ...
    def project_root_agents_state(
        self, workspace_root: Path, *, hidden: bool = False
    ) -> HostRegistrationState:
        root = _workspace_root(workspace_root)
        path = _agents_path(root)
        marker = _read_agents_marker(root)
        raw = _read_optional_regular_file(path, label="Codex AGENTS.override.md")
        if marker:
            if _git_is_tracked(root, path.relative_to(root)):
                return HostRegistrationState.FOREIGN
            return (
                HostRegistrationState.CURRENT
                if raw == _agents_file_bytes(hidden=hidden)
                else HostRegistrationState.STALE_OWNED
            )
        if raw is None:
            return HostRegistrationState.ABSENT
        return (
            HostRegistrationState.CURRENT
            if _manual_agents_override_is_current(raw, hidden=hidden)
    ...
    def project_registration_diagnostic(
        ...
        root_agents_state: HostRegistrationState | None = None
        if state is HostRegistrationState.CURRENT and not isolated_development:
            root_agents_state = self.project_root_agents_state(root, hidden=hidden)
            if root_agents_state is not HostRegistrationState.CURRENT:
                state = (
                    HostRegistrationState.STALE_OWNED
                    if root_agents_state is HostRegistrationState.ABSENT
                    else root_agents_state
                )
        ...
        if raw is not None:
            ...
            if marker is None and _manual_config_is_desired(
                ...
                return (
                    ...
                    if _reconcile_agents_file(root, hidden=hidden)
            ...
            if marker is not None and _config_is_desired(
                ...
                agents_changed = _reconcile_agents_file(root, hidden=hidden)
                if _ensure_codex_exclude(root):
                    agents_changed = True
                return IntegrationChange.CHANGED if agents_changed else IntegrationChange.UNCHANGED
    ...
    def remove_project(self, workspace_root: Path) -> IntegrationChange:
        ...
        agents_changed = _remove_agents_file(root)
        path = _config_path(root)
        marker_path = _marker_path(root)
        marker = _read_owner_marker(root)
        raw = _read_optional_regular_file(path, label="Codex project config")
...
def _agents_file_bytes(*, hidden: bool) -> bytes:
    return "".join(
        (
            _AGENTS_GENERATED_HEADER,
            "# Harness bootstrap\n\n",
            f"{codex_developer_instructions(hidden=hidden)}\n\n",
            _ROOT_AGENTS_READ_INSTRUCTION,
        )
    ).encode()


def _manual_agents_override_is_current(raw: bytes, *, hidden: bool) -> bool:
    return (
        not raw.startswith(_AGENTS_GENERATED_HEADER.encode())
        and codex_developer_instructions(hidden=hidden).encode() in raw
        and _ROOT_AGENTS_READ_INSTRUCTION.encode() in raw
        and (hidden or HIDDEN_INSTRUCTION_BODY.encode() not in raw)
    )
...
def _agents_path(root: Path) -> Path:
    return root / _AGENTS_FILE


def _agents_marker_path(root: Path) -> Path:
    return root / ".codex" / _AGENTS_OWNER_MARKER
...
def _read_agents_marker(root: Path) -> bool:
    path = _agents_marker_path(root)
    raw = _read_optional_regular_file(path, label="Codex AGENTS ownership marker")
    if raw is None:
        return False
    try:
        value = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise HostIntegrationError(f"Codex AGENTS ownership marker is malformed: {path}") from exc
    if (
        not isinstance(value, dict)
        or set(value) != {"version", "workspace_root"}
        or value.get("version") != _OWNER_VERSION
        ...
        raise HostIntegrationError(
            f"Codex AGENTS ownership marker does not match Workspace: {path}"
        )
    return True


def _preflight_agents_file(root: Path, *, hidden: bool = False, removing: bool = False) -> None:
    path = _agents_path(root)
    marker_path = _agents_marker_path(root)
    if _git_is_tracked(root, marker_path.relative_to(root)):
        raise HostIntegrationError(
            f"tracked Harness Codex AGENTS ownership marker requires manual cleanup: {marker_path}"
        )
    if not _read_agents_marker(root):
        if removing:
            return
        raw = _read_optional_regular_file(path, label="Codex AGENTS.override.md")
        if raw is not None and not _manual_agents_override_is_current(raw, hidden=hidden):
            raise HostIntegrationError(
                "existing root AGENTS.override.md shadows AGENTS.md and lacks the exact "
                "Harness bootstrap and post-status AGENTS.md read instruction, or has lost its "
                f"Harness ownership marker; repair it or move the override before retrying: {path}"
            )
        if raw is None and _git_is_tracked(root, path.relative_to(root)):
            raise HostIntegrationError(
                f"tracked root AGENTS.override.md is missing; restore it before retrying: {path}"
            )
        return
    if _git_is_tracked(root, path.relative_to(root)):
        raise HostIntegrationError(
            f"tracked Harness Codex AGENTS.override.md requires manual cleanup: {path}"
        )
    raw = _read_optional_regular_file(path, label="Codex AGENTS.override.md")
    if raw is not None and raw not in (
        _agents_file_bytes(hidden=False),
        _agents_file_bytes(hidden=True),
    ):
        raise HostIntegrationError(
            f"Harness-owned Codex AGENTS.override.md contains unknown user content: {path}"
        )


def _reconcile_agents_file(root: Path, *, hidden: bool) -> bool:
    path = _agents_path(root)
    marker_path = _agents_marker_path(root)
    desired = _agents_file_bytes(hidden=hidden)
    marker_exists = _read_agents_marker(root)
    ...
    if not marker_exists:
        marker_bytes = _owner_marker_bytes(root)
        _replace_if_unchanged(
            marker_path,
            None,
            marker_bytes,
            0o600,
            label="Codex AGENTS ownership marker",
        )
        try:
            _ensure_agents_exclude(root)
            _replace_if_unchanged(path, None, desired, 0o644, label="Codex AGENTS.override.md")
        except Exception:
            _delete_if_unchanged(marker_path, marker_bytes, label="Codex AGENTS ownership marker")
            if not _another_owned_worktree(root, agents=True):
                _remove_agents_exclude(root)
            raise
        return True
    exclude_changed = _ensure_agents_exclude(root)
    raw = _read_optional_regular_file(path, label="Codex AGENTS.override.md")
    if raw == desired:
        return exclude_changed
    _replace_if_unchanged(
        path,
        raw,
        desired,
        0o644 if raw is None else stat.S_IMODE(path.lstat().st_mode),
        label="Codex AGENTS.override.md",
    )
    return True


def _remove_agents_file(root: Path) -> bool:
    if not _read_agents_marker(root):
        return False
    path = _agents_path(root)
    if _git_is_tracked(root, path.relative_to(root)):
    ...
    if raw is not None:
        _delete_if_unchanged(path, raw, label="Codex AGENTS.override.md")
    marker_path = _agents_marker_path(root)
    marker_raw = _read_optional_regular_file(marker_path, label="Codex AGENTS ownership marker")
    if marker_raw is not None:
        _delete_if_unchanged(marker_path, marker_raw, label="Codex AGENTS ownership marker")
    if not _another_owned_worktree(root, agents=True):
        _remove_agents_exclude(root)
    return True
...
def _exclude_block(begin: str, body: tuple[str, ...], end: str) -> bytes:
    return ("\n".join((begin, *body, end)) + "\n").encode()


def _ensure_codex_exclude(root: Path) -> bool:
    return _ensure_exclude_block(root, _EXCLUDE_BEGIN, _EXCLUDE_BODY, _EXCLUDE_END)


def _ensure_agents_exclude(root: Path) -> bool:
    return _ensure_exclude_block(
        root, _AGENTS_EXCLUDE_BEGIN, _AGENTS_EXCLUDE_BODY, _AGENTS_EXCLUDE_END
    )


def _ensure_exclude_block(root: Path, begin: str, body: tuple[str, ...], end: str) -> bool:
    if not _is_git_worktree(root):
        return False
    path = _git_info_exclude(root)
    raw = _read_optional_regular_file(path, label="Git info/exclude")
    block = _exclude_block(begin, body, end)
    content = b"" if raw is None else raw
    if block in content:
        return False
    if begin.encode() in content or end.encode() in content:
        raise HostIntegrationError(f"Git info/exclude contains an ambiguous {begin} block")
...
def _remove_codex_exclude(root: Path) -> None:
    _remove_exclude_block(root, _EXCLUDE_BEGIN, _EXCLUDE_BODY, _EXCLUDE_END)


def _remove_agents_exclude(root: Path) -> None:
    _remove_exclude_block(root, _AGENTS_EXCLUDE_BEGIN, _AGENTS_EXCLUDE_BODY, _AGENTS_EXCLUDE_END)


def _remove_exclude_block(root: Path, begin: str, body: tuple[str, ...], end: str) -> None:
    if not _is_git_worktree(root):
        return
    path = _git_info_exclude(root)
    raw = _read_optional_regular_file(path, label="Git info/exclude")
...
def _another_owned_worktree(root: Path, *, agents: bool = False) -> bool:
    if not _is_git_worktree(root):
        return False
    ...
    for line in completed.stdout.splitlines():
        ...
        if other == root or not other.is_dir():
            continue
        try:
            owned = _read_agents_marker(other) if agents else _read_owner_marker(other) is not None
            if owned:
                return True
