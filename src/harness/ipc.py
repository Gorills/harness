...
class WorkspaceScanResult:
    """Bounded result returned by the daemon-owned public scan workflow."""
...
def request_workspace_scan(
    ...
    *,
    ...
    """Register/reuse one Git Workspace and reconcile its deterministic file inventory."""
    path_value = str(workspace_path)
    _validate_workspace_path(path_value)
    request_id = uuid4().hex
    response = _request_response(
        socket_path,
        {
...
def send_workspace_scan_response(
    ...
    """Send the exact success contract for the daemon-owned scan workflow."""
    peer.sendall(
        _encode_json(
            {
                "version": PROTOCOL_VERSION,
...
def _validate_workspace_path(path: str) -> None:
    if not path or len(path) > _HINT_PATH_MAX_LENGTH or "\x00" in path:
        raise IpcProtocolError("workspace path must be a non-empty bounded path")
    if not Path(path).is_absolute():
        raise IpcProtocolError("workspace path must be absolute")
...
def _workspace_scan_from_response(
    ...
    *,
    ...
    if not isinstance(project_created, bool) or not isinstance(workspace_created, bool):
        raise IpcProtocolError("daemon workspace scan creation flags have invalid field types")
...
def _bounded_response_string(value: object, field: str, maximum: int) -> str:
    if not isinstance(value, str) or not value or len(value) > maximum:
        raise IpcProtocolError(f"daemon workspace response has invalid {field}")
    return value
