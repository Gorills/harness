...
def _agents(root: Path) -> Path:
    return root / "AGENTS.override.md"


def _agents_marker(root: Path) -> Path:
    return root / ".codex" / ".harness-agents-owner.json"
...
def test_codex_project_reconcile_creates_exact_owned_config_and_git_excludes(
    ...
    assert "restart Codex in a new conversation; resume the unfinished Harness Task" in config_text
    ...
    assert os.stat(_agents_marker(root)).st_mode & 0o777 == 0o600
    assert b"project_status" in _agents(root).read_bytes()
    exclude = _exclude(root).read_text(encoding="utf-8")
    assert "# BEGIN HARNESS CODEX MCP" in exclude
    assert "# BEGIN HARNESS CODEX AGENTS" in exclude
    assert "/AGENTS.override.md" in exclude
    assert "/.codex/.harness-agents-owner.json" in exclude
...
def test_codex_preserves_existing_user_agents_file(tmp_path: Path, tracked: bool) -> None:
    root = _repository(tmp_path / "repo")
    user_agents = root / "AGENTS.md"
    user_agents.write_text("# User project instructions\n", encoding="utf-8")
    if tracked:
        _git(root, "add", "AGENTS.md")
    adapter = _adapter()

    assert adapter.reconcile_project(root, hidden=True) is IntegrationChange.CHANGED
    assert user_agents.read_text(encoding="utf-8") == "# User project instructions\n"
    assert _agents_marker(root).is_file()
    assert "After `project_status`, if a root `AGENTS.md` exists" in _agents(root).read_text(
        encoding="utf-8"
    )
    ...
    assert user_agents.read_text(encoding="utf-8") == "# User project instructions\n"
    assert not _agents(root).exists()


@pytest.mark.parametrize("tracked", [False, True])
def test_codex_refuses_root_user_override_without_bootstrap_before_config_change(
    tmp_path: Path, tracked: bool
) -> None:
    root = _repository(tmp_path / "repo")
    override = _agents(root)
    override.write_text("# User override\n", encoding="utf-8")
    if tracked:
        _git(root, "add", "AGENTS.override.md")

    with pytest.raises(HostIntegrationError, match=r"shadows AGENTS.md"):
        _adapter().reconcile_project(root)
    assert override.read_text(encoding="utf-8") == "# User override\n"
    assert not _config(root).exists()
    assert not _agents_marker(root).exists()


def test_codex_preserves_manual_root_override_with_exact_bootstrap(tmp_path: Path) -> None:
    root = _repository(tmp_path / "repo")
    override = _agents(root)
    body = (
        f"# User override\n\n{CODEX_BOOTSTRAP_INSTRUCTION_BODY}\n\n"
        "After `project_status`, if a root `AGENTS.md` exists, read it before any other "
        "repository work and follow its instructions.\n"
    )
    ...
    assert adapter.reconcile_project(root) is IntegrationChange.CHANGED
    assert override.read_text(encoding="utf-8") == body
    assert not _agents_marker(root).exists()
    assert adapter.remove_project(root) is IntegrationChange.CHANGED
    assert override.read_text(encoding="utf-8") == body


def test_codex_refuses_to_leave_manual_hidden_override_active_on_normal_transition(
    tmp_path: Path,
) -> None:
    root = _repository(tmp_path / "repo")
    adapter = _adapter()
    assert adapter.reconcile_project(root, hidden=True) is IntegrationChange.CHANGED
    manual = _agents(root).read_bytes()
    _agents_marker(root).unlink()
    _agents(root).write_bytes(
        manual.removeprefix(
            b"<!-- Generated by Harness; ownership: .codex/.harness-agents-owner.json. -->\n"
        )
    )
    config_before = _config(root).read_bytes()

    with pytest.raises(HostIntegrationError, match=r"shadows AGENTS.md"):
        adapter.reconcile_project(root, hidden=False)
    assert _config(root).read_bytes() == config_before


def test_codex_refuses_generated_override_with_missing_owner_marker(tmp_path: Path) -> None:
    root = _repository(tmp_path / "repo")
    adapter = _adapter()
    assert adapter.reconcile_project(root) is IntegrationChange.CHANGED
    _agents_marker(root).unlink()

    assert adapter.project_root_agents_state(root) is HostRegistrationState.FOREIGN
    assert adapter.project_registration_state(root) is HostRegistrationState.FOREIGN
    with pytest.raises(HostIntegrationError, match="lost its Harness ownership marker"):
        adapter.reconcile_project(root)
    assert _agents(root).is_file()


def test_codex_owned_agents_file_reconciles_hidden_and_stays_ignored(tmp_path: Path) -> None:
    root = _repository(tmp_path / "repo")
    adapter = _adapter()

    assert adapter.reconcile_project(root) is IntegrationChange.CHANGED
    normal = _agents(root).read_bytes()
    assert HIDDEN_INSTRUCTION_BODY.encode() not in normal
    assert adapter.reconcile_project(root, hidden=True) is IntegrationChange.CHANGED
    hidden = _agents(root).read_bytes()
    assert HIDDEN_INSTRUCTION_BODY.encode() in hidden
    assert _git(root, "check-ignore", "-q", "AGENTS.override.md").returncode == 0
    assert _git(root, "status", "--porcelain", "--untracked-files=all").stdout == ""
    assert adapter.reconcile_project(root, hidden=False) is IntegrationChange.CHANGED
    assert _agents(root).read_bytes() == normal


def test_codex_diagnostic_detects_missing_root_override(tmp_path: Path) -> None:
    root = _repository(tmp_path / "repo")
    adapter = _adapter()
    assert adapter.reconcile_project(root) is IntegrationChange.CHANGED
    _agents(root).unlink()

    assert adapter.project_root_agents_state(root) is HostRegistrationState.STALE_OWNED
    assert adapter.project_registration_state(root) is HostRegistrationState.STALE_OWNED
    assert adapter.reconcile_project(root) is IntegrationChange.CHANGED
    assert adapter.project_registration_state(root) is HostRegistrationState.CURRENT


def test_codex_refuses_modified_owned_agents_file_before_config_change(tmp_path: Path) -> None:
    root = _repository(tmp_path / "repo")
    adapter = _adapter()
    assert adapter.reconcile_project(root) is IntegrationChange.CHANGED
    config_before = _config(root).read_bytes()
    _agents(root).write_text("# User changes\n", encoding="utf-8")
    ...
    assert _agents(root).read_text(encoding="utf-8") == "# User changes\n"


def test_codex_agents_appearance_during_reconcile_preserves_user_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = _repository(tmp_path / "repo")
    original = codex_module._replace_if_unchanged

    def appear(
        path: Path, expected: bytes | None, replacement: bytes, mode: int, *, label: str
    ) -> None:
        if path == _agents(root) and expected is None:
            path.write_text("# User file\n", encoding="utf-8")
        original(path, expected, replacement, mode, label=label)

    monkeypatch.setattr(codex_module, "_replace_if_unchanged", appear)
    with pytest.raises(HostRegistrationCollisionError, match="appeared before mutation"):
        _adapter().reconcile_project(root)
    assert _agents(root).read_text(encoding="utf-8") == "# User file\n"
    assert not _agents_marker(root).exists()
    assert "# BEGIN HARNESS CODEX AGENTS" not in _exclude(root).read_text(encoding="utf-8")
...
def test_codex_exact_tracked_manual_config_is_adopted_but_never_removed(tmp_path: Path) -> None:
    ...
    assert diagnostic.preflight_error is None
    assert generated.reconcile_project(root) is IntegrationChange.CHANGED
    assert generated.remove_project(root) is IntegrationChange.CHANGED
...
def test_codex_linked_worktrees_keep_distinct_roots_and_shared_exclude_until_last_remove(
    ...
    assert "# BEGIN HARNESS CODEX AGENTS" in exclude.read_text(encoding="utf-8")
    ...
    assert "# BEGIN HARNESS CODEX AGENTS" not in exclude.read_text(encoding="utf-8")
