...
The implemented Linux/POSIX installation slice supports Claude Code plus local Cursor IDE/CLI. `harness install --host claude-code|cursor|all` performs runtime and registration-ownership preflight, replaces a stale daemon only through the frozen schema/package-version/interpreter/code identity contract, and then delegates host mutation to the selected adapters. Omitted `--host` remains Claude Code for compatibility. Claude uses the official `claude mcp` CLI and `CLAUDE_PROJECT_DIR`. Cursor production MCP is project-only: every registered Workspace `.cursor/mcp.json` launches the installed Python with `-m harness.mcp_process`, `HARNESS_HOST_PROFILE=cursor`, and `HARNESS_WORKSPACE_ROOT=${workspaceFolder}`. Harness records Cursor intent in host integration state next to the canonical database and removes leftover owned global `user-harness`. A foreign global `harness` is a fail-closed collision. After writing a project config, install/scan run official `agent mcp enable harness` and verify `agent mcp list-tools harness` for the exact five tools; missing `agent` prints `cd <workspace> && agent mcp enable harness` plus a full Cursor quit/reopen. Cursor IDE may inject `WORKSPACE_FOLDER_PATHS` on a leftover shared process; it is not Workspace identity. Cursor-profile Workspace resolution uses only interpolated `HARNESS_WORKSPACE_ROOT` and never falls back to cwd, an uninterpolated `${workspaceFolder}` literal, or `WORKSPACE_FOLDER_PATHS`. A Cursor-profile process without an interpolated root lists no tools. Tracked Cursor project config is manual-adoption/removal only; Harness-created untracked config carries Workspace-local ownership metadata and Git-local exclusions. The Harness source checkout itself commits a tracked isolated-development overlay named `harness-dev` that launches `scripts/dev harness mcp` instead of the production Cursor signature; production install/scan/uninstall must not rewrite, delete, or enable it as production `harness`, a system `harness scan` of that overlay root is refused, and isolated `harness install`/`uninstall` are refused while `HARNESS_DEV_ROOT` is set. A production MCP process with `HARNESS_HOST_PROFILE` lists no tools and refuses calls when its interpolated `HARNESS_WORKSPACE_ROOT` is that overlay, including when `WORKSPACE_FOLDER_PATHS` names a working repository; Cursor-profile overlay refuse does not use cwd. The overlay launch without that profile marker still exposes the five Harness tools.

`harness scan` inspects Harness host integration state rather than global Cursor MCP presence, reconciles the scanned Cursor project override when Cursor is active, enables/verifies that project MCP, and submits one combined host-profile set to daemon-owned skill reconciliation. This keeps Cursor compatibility roots from producing duplicate generated skills. `harness uninstall` removes only selected host artifacts. If another supported host remains, every registered live Workspace is reconciled against the remaining profile set and the shared daemon stays running; otherwise owned skills are cleaned and the daemon shuts down. `--purge` is refused while another supported host remains active. Bare `harness doctor` reports Claude registration, leftover/foreign/absent global Cursor MCP, on-disk project overrides, Cursor approval/tool catalog, daemon runtime, and Project index as separate results. Isolated doctor does not inspect user-global Cursor/Claude MCP or `~/.harness/skills`. Core Task/Knowledge/index logic remains host-neutral. This is automated local integration proof, not proprietary-host UI/CLI acceptance; Cursor Cloud Agents remain a separate unsupported profile.
