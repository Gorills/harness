...
The implemented Linux/POSIX installation slice currently supports Claude Code only. `harness install` performs runtime and registration-ownership preflight, compares the daemon's frozen schema/package-version/interpreter/code fingerprint with the currently installed package, cleanly replaces stale loaded runtime code when necessary, prepares the canonical daemon/database, and delegates the user-scope MCP mutation to the Claude adapter. `harness scan` may request daemon-owned relevant-skill reconciliation only when that adapter reports the exact current Harness registration. `harness uninstall` first reconciles Harness-owned Claude skill projections to an empty set, then removes the owned MCP registration and requests clean daemon shutdown; the canonical database survives unless the operator explicitly passes `--purge`. Cleanup never edits proprietary host configuration directly and never recursively deletes unknown project or registry content. Bare `harness doctor` is the read-only operational diagnostics boundary for this profile: it inspects canonical path safety, host registration ownership, frozen daemon runtime identity, authoritative Project/Workspace identity, live deterministic index freshness, expected generated skills, and dashboard subsystem state without starting/reconciling those systems. `--runtime-only` retains the ephemeral SQLite probe. This is installation/diagnostic mechanics proof, not proprietary-host acceptance.
