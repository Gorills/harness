...
## Mandatory Harness bootstrap for agents

- `project_status` must be the first repository action in every agent Task. Before any shell
  command, repository file read/search, browser inspection, or change, locate the configured
...
- Cursor's tracked config names `harness-dev` and launches `scripts/dogfood mcp`. Codex instead
  uses an ignored, mode-0600 `.codex/config.toml` generated by the selected daemon installation;
  its authenticated loopback HTTP endpoint carries the exact checkout root. Prefer reading this
  repository over broad Harness context when implementing Harness.
- Never invoke a leftover user-level/global MCP namespace (`user-harness` or equivalent) from this
  ...
  project `harness-dev` in Cursor Customize; Codex must have only generated `harness`, never legacy
  `harness-dev`. `harness scan --global-dogfood` is reserved for the
