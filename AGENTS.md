...
- After status, use `project_search` and expand only selected refs with `project_context` before
  targeted native tools. Start or resume a Harness Task before changes and checkpoint meaningful
  progress.
...
- Cursor's tracked config names `harness-dev`; `.mcp.json` still names `harness` for Claude Code.
  Those stdio overlays launch `scripts/dogfood mcp`. Codex instead uses an ignored, mode-0600
  `.codex/config.toml` generated by the selected daemon installation; its authenticated loopback
  HTTP endpoint carries the exact checkout root. Prefer reading this repository over broad Harness
  context when implementing Harness.
