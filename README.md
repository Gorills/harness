...
> **Repository status:** implementation foundation. Packaging/tooling, SQLite persistence, Project/Workspace registry primitives, read-only runtime doctor checks, Workspace status and deterministic scan CLIs, bounded daemon/local-IPC status and scan paths, POSIX daemon singleton/stale-socket recovery, canonical POSIX lazy daemon autostart, deterministic file-index reconciliation, and canonical POSIX per-user runtime paths exist; MCP, search, Tasks, watcher, host integration, and dashboard behavior are not implemented yet.
