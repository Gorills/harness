...
> **Repository status:** implementation foundation. Packaging/tooling, SQLite persistence, Project/Workspace registry primitives, read-only runtime doctor checks, a bounded daemon/local-IPC status path, and the first deterministic file-index reconciliation primitives exist; MCP, search, Tasks, host integration, and dashboard behavior are not implemented yet.
