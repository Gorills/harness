...
> **Repository status:** implementation foundation. Packaging/tooling, SQLite persistence, Project/Workspace registry primitives, read-only runtime doctor checks, Workspace status and deterministic scan CLIs, bounded daemon/local-IPC status and scan paths, POSIX daemon singleton/stale-socket recovery, canonical POSIX lazy daemon autostart, deterministic file-index reconciliation, an internal bounded indexed-path search primitive, and canonical POSIX per-user runtime paths exist; public search exposure, MCP, Tasks, watcher, host integration, and dashboard behavior are not implemented yet.
...
The first search-domain slice performs deterministic, Workspace-scoped retrieval over current `indexed_files` rows without reading or returning source text. It ranks exact relative-path matches, exact filenames, normalized ASCII identifier-token matches (including common camel/snake/kebab/dotted forms), then case-insensitive path substrings. Queries are bounded to 256 UTF-8 bytes, result count is hard-bounded to 50 with a default of 10, and each hit exposes only relative path, entry kind, size, and the mechanical match reason. This is deliberately an internal domain primitive: daemon IPC, CLI/MCP `project_search`, FTS/content search, symbols, docs, Git retrieval, Tasks, Knowledge, Working Sets, and rank fusion remain separate later slices.
