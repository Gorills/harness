...
> **Repository status:** implementation foundation. Packaging/tooling, SQLite persistence, Project/Workspace registry primitives, read-only runtime doctor checks, a read-only Workspace status CLI, bounded daemon/local IPC status paths, deterministic file-index reconciliation, a public daemon-owned `harness scan` workflow, and canonical POSIX per-user runtime paths exist; MCP, search, Tasks, host integration, watcher automation, and dashboard behavior are not implemented yet.
...
The installed CLI also exposes `harness scan [PATH] [--socket PATH]`. `PATH` defaults to the current directory and may point anywhere inside a Git worktree. The CLI sends the resolved absolute location to `harnessd`; the daemon owns both durable registration and index mutation. The first scan creates a Normal Project and Workspace when that Git common directory is not yet known, or reuses the unique existing Project for another linked worktree, then reconciles the deterministic file inventory. Repeat scans are idempotent with respect to registry identity and report added/updated/removed counts. `--socket` remains an explicit development/recovery override; without it the CLI validates and uses the canonical per-user socket.
...
The current internal IPC protocol is independent of MCP wire objects, uses protocol version `1`, allows exactly one request/response per connection, enforces a 16 KiB message limit, uses bounded client/server timeouts, and rejects unknown fields/methods fail-closed. `workspace_status` is read-only. The additive `workspace_scan` method accepts exactly one bounded absolute path and is the first mutating IPC path; all registration and index writes remain daemon-owned. Its result is bounded to registry identity, visibility, creation flags, schema version, and reconciliation counts. The original `status` and `workspace_status` wire shapes remain unchanged. This slice still does not implement daemon autostart/service management, expose MCP, or claim a verified Windows named-pipe/equivalent transport; Windows daemon IPC and canonical Windows transport paths therefore remain unsupported until that separate platform task is proven.
