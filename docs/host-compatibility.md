...
| Cursor | `~/.cursor/mcp.json` | `.cursor/mcp.json` | `${workspaceFolder}` in both global and project entries, mapped to exact `HARNESS_WORKSPACE_ROOT`; Cursor IDE interpolates the user-level value from the current window | `.agents/skills/`, `.cursor/skills/`, plus compatibility roots | `~/.agents/skills/`, `~/.cursor/skills/`, plus compatibility roots | Local IDE + CLI adapter implemented; Cursor Cloud Agents are a separate out-of-scope profile. |
...
The production Linux Cursor adapter edits only Cursor's documented local JSON MCP surfaces. `~/.cursor/mcp.json` contains the global Harness stdio entry using the exact installed Python, `-m harness.mcp_process`, `HARNESS_HOST_PROFILE=cursor`, and `HARNESS_WORKSPACE_ROOT=${workspaceFolder}`. Every registered Workspace also receives a complete same-name `.cursor/mcp.json` override with that same contract so a later-approved project identifier is not a different launch. When the bridge sees the Cursor profile marker, the interpolated environment value is mandatory and becomes an exact `ROOT` hint; cwd, an uninterpolated `${workspaceFolder}` literal, and inherited folder-path environment are never Workspace identity. A Cursor-profile process without that interpolated hint lists no tools and refuses calls. If that production process is nevertheless launched against the Harness source checkout's isolated-development overlay, it lists no tools and refuses calls; overlay detection consults cwd when `HARNESS_WORKSPACE_ROOT` is absent or unresolvable.
