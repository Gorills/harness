...
| Cursor | `~/.cursor/mcp.json` | `.cursor/mcp.json` | Project `${workspaceFolder}` mapped to `HARNESS_WORKSPACE_ROOT`; user-level `user-harness` uses Cursor-injected `WORKSPACE_FOLDER_PATHS`. Cursor IDE `user-harness` is profile-scoped shared process | `.agents/skills/`, `.cursor/skills/`, plus compatibility roots | `~/.agents/skills/`, `~/.cursor/skills/`, plus compatibility roots | Local IDE + CLI adapter implemented; Cursor Cloud Agents are a separate out-of-scope profile. |
...
The production Linux Cursor adapter edits only Cursor's documented local JSON MCP surfaces. `~/.cursor/mcp.json` contains the global Harness stdio entry using the exact installed Python, `-m harness.mcp_process`, and `HARNESS_HOST_PROFILE=cursor` without `HARNESS_WORKSPACE_ROOT`. Every registered Workspace also receives a complete same-name `.cursor/mcp.json` override with `HARNESS_WORKSPACE_ROOT=${workspaceFolder}` so an enabled project identifier is not a different launch. When the bridge sees the Cursor profile marker, interpolated `HARNESS_WORKSPACE_ROOT` is an exact `ROOT` hint when it is a real non-overlay directory; otherwise Cursor-injected `WORKSPACE_FOLDER_PATHS` is used. cwd and an uninterpolated `${workspaceFolder}` literal are never Workspace identity. A Cursor-profile process with neither signal lists no tools. If a production process has an interpolated root that is the Harness source checkout's isolated-development overlay and has no non-overlay folder-path, it lists no tools and refuses calls; Cursor-profile overlay refuse does not consult cwd. The checkout overlay is named `harness-dev`.
