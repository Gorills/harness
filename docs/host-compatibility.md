...
| Cursor | `~/.cursor/mcp.json` | `.cursor/mcp.json` | Project `${workspaceFolder}` mapped to exact `HARNESS_WORKSPACE_ROOT`; global user-level entry has no Workspace root. Cursor IDE `user-harness` is profile-scoped shared process | `.agents/skills/`, `.cursor/skills/`, plus compatibility roots | `~/.agents/skills/`, `~/.cursor/skills/`, plus compatibility roots | Local IDE + CLI adapter implemented; Cursor Cloud Agents are a separate out-of-scope profile. |
...
- Global MCP config: `~/.cursor/mcp.json`; project config: `.cursor/mcp.json`. Global and project MCP servers are merged. Official notes say a same-name project server has priority; real-host Cursor IDE evidence from 2026-08-27 is that the agent catalog still exposes the global entry as a distinct `user-harness` namespace while the project override stays disconnected until `approvedProjectMcpServers` records it. The user-level server is profile-scoped (`mcpScope:profile`, `mcp_version=shared_process`).
...
- Real-host Cursor IDE user-level stdio evidence from 2026-08-27: process cwd is the user home, not the Workspace, and inherited `WORKSPACE_FOLDER_PATHS` can name a different open window. Those values are not Workspace identity.
...
The production Linux Cursor adapter edits only Cursor's documented local JSON MCP surfaces. `~/.cursor/mcp.json` contains the global Harness stdio entry using the exact installed Python, `-m harness.mcp_process`, and `HARNESS_HOST_PROFILE=cursor` without `HARNESS_WORKSPACE_ROOT`. Every registered Workspace also receives a complete same-name `.cursor/mcp.json` override with `HARNESS_WORKSPACE_ROOT=${workspaceFolder}` so an enabled project identifier is not a different launch. When the bridge sees the Cursor profile marker, the interpolated environment value is mandatory and becomes an exact `ROOT` hint; cwd, an uninterpolated `${workspaceFolder}` literal, and inherited folder-path environment are never Workspace identity. A Cursor-profile process without that interpolated hint lists no tools and tells the model to enable the project `harness` MCP in Customize. If a production process has an interpolated root that is the Harness source checkout's isolated-development overlay, it lists no tools and refuses calls; Cursor-profile overlay refuse does not consult cwd. The checkout overlay is named `harness-dev`.
