...
checkout daemon fails session startup instead of silently removing Harness. The project must be
trusted, and Codex must be restarted after the config changes. Production Workspaces still use
the absolute project-scoped `harness` config generated by the installed adapter.
...
- Codex: `.codex/config.toml` names `harness-dev` and launches `./scripts/dev harness mcp` with a
