...
| Host MCP | project-only Codex/Cursor config plus Claude user-scope MCP | tracked project overlays launching `scripts/dogfood mcp` (`harness-dev` in Codex/Cursor); isolated by default |
...
The tracked Codex, Cursor, and Claude project MCP entries launch `scripts/dogfood`. With no marker,
...
`./scripts/dogfood mcp`, allows 30 seconds for first startup, and is required so a broken selected
route fails session startup instead of silently removing Harness. The project must be trusted, and
Codex must be restarted after the config or dogfood mode changes. Production Workspaces still use
the absolute project-scoped `harness` config generated by the installed adapter. The checkout
config contains no second `harness` alias, so Codex sees one Harness tool catalog.
...
This checkout commits host overlays that do not use the production Codex/Cursor/Claude adapter signature (`python -m harness.mcp_process` plus `HARNESS_HOST_PROFILE`):
...
- Codex: `.codex/config.toml` names `harness-dev` and launches `./scripts/dogfood mcp` with a
  relative checkout root resolved from the project-scoped MCP process working directory.
