...
A production MCP process (`HARNESS_HOST_PROFILE` set, typically `python -m harness.mcp_process`) must not expose tools when it is launched against that overlay checkout. The Cursor documented root hint is `HARNESS_WORKSPACE_ROOT`; Claude Code's is `CLAUDE_PROJECT_DIR`. Process cwd is consulted when that hint is absent or does not resolve to an existing path, which covers the global Cursor user-server case, and is not Workspace identity. The tracked overlay without `HARNESS_HOST_PROFILE` still serves the five tools against checkout-local state. If a host launches the user-global server with a cwd that is not the overlay worktree, this refuse does not activate.
