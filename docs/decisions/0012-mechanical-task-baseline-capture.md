...
New baseline-aware Task creation runs in one `BEGIN IMMEDIATE` transaction:
...
A capture or persistence failure therefore leaves no newly created Task. The baseline is part of creation, not a later existing-Task mutation, so it does not consume a second revision.

The earlier `create_task_record` primitive remains foundation-only for low-level tests and internal migration coverage. Future `task_start` creation must use the baseline-aware path.
...
- A crash or capture race cannot leave a newly started Task without its required baseline.
...
- baseline-aware Task creation writes Task + baseline atomically at revision `1`;
