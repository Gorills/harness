...
6. The tracked Cursor and Claude MCP configurations launch the router. Codex uses one ignored,
   mode-0600, production-shaped `harness` HTTP config generated by the selected daemon install.
   The legacy Codex `harness-dev` stdio overlay is accepted only as migration input and is removed;
   duplicate catalogs remain forbidden because they make tool selection and diagnostics ambiguous.
...
- tracked Cursor/Claude configs expose one router-backed source-checkout server while Codex exposes
  one generated authenticated HTTP server;
- global doctor recognizes the tracked Cursor source-checkout overlay and generated Codex config as
  intentional and does not require skill projections that the index-only route deliberately skips;
